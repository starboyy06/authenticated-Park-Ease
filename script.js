...
// Check-in vehicle - MODIFIED FOR EV CHARGING
function checkInVehicle() {
    ...
    // MODIFIED: Calculate costs separately for EV charging
    let parkingCost, chargingCost, expectedCost;
    if (vehicleType === 'charging') {
        parkingCost = parkingData.rates.car * duration; // ₹60/hour for parking
        chargingCost = parkingData.rates.charging * duration; // ₹300/hour for charging
        expectedCost = parkingCost + chargingCost;
    } else {
        parkingCost = parkingData.rates[vehicleType] * duration;
        chargingCost = 0;
        expectedCost = parkingCost;
    }
    ...
    updateStats(vehicleType);
    generateReceipt(carNumber, availableSlot, vehicleType, checkInTime, parkingCost, chargingCost, expectedCost);
...
// Check-out vehicle - MODIFIED FOR EV CHARGING
function checkOutVehicle() {
    ...
    let billedMinutes, actualHours, parkingCost, chargingCost;

    // MODIFIED: Handle charging vehicles with separate costs
    if (vehicleType === 'charging') {
        // For charging vehicles, calculate both parking and charging costs
        if (diffMinutes <= 60) {
            billedMinutes = 60;
            actualHours = 1.00;
            parkingCost = parkingData.rates.car; // ₹60 for parking
            chargingCost = parkingData.rates.charging; // ₹300 for charging
        } else {
            ...
            // Calculate both parking and charging costs
            const additionalHours = additionalBilledMinutes / 60;
            parkingCost = parkingData.rates.car + (parkingData.rates.car * additionalHours);
            chargingCost = parkingData.rates.charging + (parkingData.rates.charging * additionalHours);
        }
    } else {
        // For regular parking vehicles
        if (diffMinutes <= 60) {
            billedMinutes = 60;
            actualHours = 1.00;
            parkingCost = parkingData.rates[vehicleType];
            chargingCost = 0;
        } else {
            const extraMinutes = diffMinutes - 60;
            const incrementMinutes = parkingData.billingIncrement;
            ...
            billedMinutes = 60 + additionalBilledMinutes;
            actualHours = (billedMinutes / 60).toFixed(2);
            parkingCost = parkingData.rates[vehicleType] + (parkingData.rates[vehicleType] * (additionalBilledMinutes / 60));
            chargingCost = 0;
        }
    }
    ...
    showAlert(`Vehicle ${carNumber} successfully checked out from slot ${slotNumber}! Parking cost: ₹${parkingCost.toFixed(2)}, Charging cost: ₹${chargingCost.toFixed(2)}, Total cost: ₹${totalCost.toFixed(2)}`, 'success');
...
// Generate check-in receipt - MODIFIED FOR EV CHARGING
function generateReceipt(carNumber, slotNumber, vehicleType, checkInTime, parkingCost, chargingCost, totalCost) {
    const receiptHTML = `
        <div class="receipt" id="checkinReceipt">
            ...
            <div class="receipt-item">
                ...
                <span>₹${parkingData.rates.car}/hour</span>
            </div>
            <div class="receipt-item">
            ...
            <div class="receipt-item">
                <span>Expected Parking Cost:</span>
                <span>₹${parkingCost}</span>
            </div>
            <div class="receipt-item">
                <span>Expected Charging Cost:</span>
...
// Generate checkout receipt - MODIFIED FOR EV CHARGING
function generateCheckoutReceipt(carNumber, slotNumber, vehicleType, checkInTime, checkOutTime, durationDisplay, billedHours, parkingCost, chargingCost, totalCost) {
    ...
    const receiptHTML = `
        <div class="receipt" id="checkoutReceipt">
            ...
            <div class="receipt-item">
                <span>Parking Rate:</span>
                <span>₹${parkingData.rates[safeVehicleType]}/hour</span>
            </div>
            <div class="receipt-item">
                <span>Parking Cost:</span>
                <span>₹${safeParkingCost.toFixed(2)}</span>
...
// Charge EV - MODIFIED FOR SEPARATE COSTS
function chargeEV() {
    ...
    // MODIFIED: Calculate both parking and charging costs
    const parkingCost = parkingData.rates.car * duration; // ₹60/hour for parking
    const chargingCost = parkingData.rates.charging * duration; // ₹300/hour for charging
    const totalCost = parkingCost + chargingCost;
    ...
    generateChargingReceipt(carNumber, availableSlot, duration, parkingCost, chargingCost, totalCost, chargeTime);
    
    showAlert(`Vehicle ${carNumber} charged in slot ${availableSlot} for ${duration} hours. Parking: ₹${parkingCost}, Charging: ₹${chargingCost}, Total: ₹${totalCost}`, 'success');
...
// Generate charging receipt - MODIFIED FOR SEPARATE COSTS
function generateChargingReceipt(carNumber, chargingSlot, duration, parkingCost, chargingCost, totalCost, chargeTime) {
    const receiptHTML = `
        ...
        reader.onload = function(e) {
            try {
                ...
                if (data.parkedVehicles) {
                    ...
                    // Update slots with imported data
...
function updateChargingCost() {
    ...
    const chargingCost = parkingData.rates.charging * duration; // ₹300/hour for charging
    const totalCost = parkingCost + chargingCost;
    document.getElementById('chargingCostDisplay').textContent = `Cost: Parking ₹${parkingCost} + Charging ₹${chargingCost} = Total ₹${totalCost}`;
}